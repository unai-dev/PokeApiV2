    ...
    public class TrainerController : ControllerBase
    {
        ...
        public async Task<ActionResult> Post(AddTrainerDTO trainerDTO)
        {
            var pokemonsIds = await context.Pokemons.Where(x =>
                trainerDTO.PokemonIds.Contains(x.Id)).Select(x => x.Id).ToListAsync();

            if (pokemonsIds.Count != trainerDTO.PokemonIds.Count)
            {
                var pokemonsNotFound = trainerDTO.PokemonIds.Except(pokemonsIds);
                var pokemonsString = string.Join(",", pokemonsNotFound);
                var errorMessage = $"This pokemons not found {pokemonsString}";

                ModelState.AddModelError(nameof(trainerDTO.PokemonIds), errorMessage);
